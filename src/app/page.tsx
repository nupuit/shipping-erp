...
type Party = { id: string; party_name: string; party_type: string; contact_person: string; email: string; address: string };
type Vessel = { id: string; vessel_name: string; imo_no: string; voyage: string; capacity_teu: number; flag: string };
type Commodity = { id: string; commodity_code: string; description: string; hs_code: string };
type Port = { id: string; port_code: string; port_name: string; country: string; city: string; timezone: string };
...
export default function Home() {
  ...
  const [newParty, setNewParty] = useState({ party_name: "", party_type: "Agent", contact_person: "", email: "", address: "" });
  ...
  const [items, setItems] = useState<BookingItem[]>([{ size: "", item_type: "", quantity: 1, gross_weight: "", packages: "", unit: "", cargo_volume: "" }]);
  ...
  async function fetchAllData() {
    await Promise.all([fetchParties(), fetchVessels(), fetchCommodities(), fetchPorts(), fetchBookings()]);
  }
  ...
  const validateBooking = () => {
    ...
    if (!booking.pol) return "POL is required.";
    if (!booking.pod) return "POD is required.";
    ...
    for (const [index, item] of items.entries()) {
      if (!item.size || !item.item_type || !item.gross_weight || !item.packages || !item.unit) {
  ...
  async function addParty() {
    ...
    if (error) {
      ...
      setNewParty({ party_name: "", party_type: "Agent", contact_person: "", email: "", address: "" });
  ...
  const addItem = () => setItems((prev) => [...prev, { size: "", item_type: "", quantity: 1, gross_weight: "", packages: "", unit: "", cargo_volume: "" }]);
  ...
  async function saveBooking() {
    ...
    const { error } = await supabase.from("bookings").insert([
      {
        ...
        container_owner: booking.container_owner,
        ...
        detention_currency: booking.detention_currency,
        ...
        srr: booking.srr,
        service1: booking.service1,
        service2: booking.service2,
        cargo_details: items,
    ...
    if (error) {
      ...
      setItems([{ size: "", item_type: "", quantity: 1, gross_weight: "", packages: "", unit: "", cargo_volume: "" }]);
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "140px 1fr 140px 1fr", gap: "12px", alignItems: "center" }}>
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <select value={booking.carrier_id} onChange={(e) => updateField("carrier_id", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                          ...
                          {parties.filter((p) => p.party_type === "Carrier").map((party) => (
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <select value={booking.container_owner} onChange={(e) => updateField("container_owner", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <select value={booking.pol} onChange={(e) => updateField("pol", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                          <option value="">Select POL</option>
                          {ports.map((port) => (
                            <option key={port.id} value={port.port_code}>{port.port_code} - {port.port_name}</option>
                          ))}
                        </select>
                      </label>
                      <label style={{ display: "contents" }}>
                        <div style={{ color: "#4a5b72", fontWeight: 600, marginBottom: "6px" }}>POD</div>
                        <select value={booking.pod} onChange={(e) => updateField("pod", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <select value={booking.shipper_id} onChange={(e) => updateField("shipper_id", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                          ...
                          {parties.filter((p) => p.party_type === "Shipper").map((party) => (
                            <option key={party.id} value={party.id}>{party.party_name}</option>
                          ))}
                        </select>
                      </label>
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <select value={booking.consignee_id} onChange={(e) => updateField("consignee_id", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                          ...
                          {parties.filter((p) => p.party_type === "Consignee").map((party) => (
                            <option key={party.id} value={party.id}>{party.party_name}</option>
                          ))}
                        </select>
                      </label>
                      ...
                      <label style={{ display: "contents" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                          <select value={booking.service1} onChange={(e) => updateField("service1", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                            <option>CY</option>
                            <option>Door</option>
                            <option>Port</option>
                          </select>
                          <select value={booking.service2} onChange={(e) => updateField("service2", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                            <option>CY</option>
                            <option>DOOR</option>
                            <option>OTH</option>
                          </select>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                        ...
                        <label style={{ display: "contents" }}>
                          ...
                          <select value={booking.detention_currency} onChange={(e) => updateField("detention_currency", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                            <option>USD</option>
                            <option>BDT</option>
                          </select>
                        </label>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                        ...
                        <label style={{ display: "contents" }}>
                          ...
                          <select value={booking.cc_agent_id} onChange={(e) => updateField("cc_agent_id", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                            ...
                            {parties.filter((p) => p.party_type === "Agent").map((party) => (<option key={party.id} value={party.id}>{party.party_name}</option>))}
                          </select>
                        </label>
                        <label style={{ display: "contents" }}>
                          <div style={{ color: "#4a5b72", fontWeight: 600, marginBottom: "6px" }}>Approval Status</div>
                          <select value={booking.approval_status} onChange={(e) => updateField("approval_status", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px solid #cbd5e1" }}>
                            <option>Draft</option>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#eef4ff" }}>
                        {["Size", "Type", "Quantity", "Gross Weight", "Packages", "Unit", "Cargo Volume", "Actions"].map((header) => (
                    ...
                    <tbody>
                      {items.map((item, index) => (
                        <tr key={index} style={{ borderBottom: "1px solid #e7edf5" }}>
                          <td style={{ padding: "10px" }}><input value={item.size} onChange={(e) => updateItem(index, "size", e.target.value)} style={{ width: "100%", padding: "8px 10px", borderRadius: "10px", border: "1px solid #cbd5e1" }} /></td>
                          <td style={{ padding: "10px" }}><input value={item.item_type} onChange={(e) => updateItem(index, "item_type", e.target.value)} style={{ width: "100%", padding: "8px 10px", borderRadius: "10px", border: "1px solid #cbd5e1" }} /></td>
                  ...
                  <tbody>
                    {parties.map((party) => (
                      <tr key={party.id} style={{ borderBottom: "1px solid #e7edf5" }}>
                        ...
                        <td style={{ padding: "12px 10px" }}>{party.party_type}</td>
